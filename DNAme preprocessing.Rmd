...
This script outlines key steps utilized in cleaning a DNA methylation dataset obtained from Illumina Infinium Methylation Arrays.
...
NOTE: All samples pass this check 
...
NOTE: Reported males and females cluster accordingly. Reported and predicted sex (minfi) match 
...
# pulling XY probes to check the clustering - snaity check
...
NOTE: Samples look ok overall apart from one boarderline sample (GSM3030041). We'll keep an eye on this sample in other QC checks 
```{r average intensity}
...
NOTE: GSM3030041 detected as an outlier. This sample alsp failed the intensity check 
```{r outlier}
betas <- getBeta(RGset)
detout <- outlyx(betas)
...
NOTE:GSM3030042 and GSM3030043 seem to be replicates. Only one sample will be retained for statistical modeling but we will use the correlation between this pair to monitor preprocessing
...
We'll perform noob to correct for color bias and BMIQ to account for probe-type differences
