...
con <- dbConnect(drv, dbname = "genomic_variants", host = "localhost", user = "rasiimwe", password = pw)
rm(pw)
...
#for every path in museq 3, read the file that ends wth flagcosmic.vcf
...
for(i in museqsnvs_path){
		...
		vcf <- readVcf(f, "hg19")# museq
        	  	...
        	  	##all <- data.frame(info(vcf)) #everything unde
...
#***************************************************************************************************
#Error in `$<-.data.frame`(`*tmp*`, "tumour_archive_id", value = "SA218") :
#replacement has 1 row, data has 30383
#the error above is being generated by museq_info -> try to understand why
#***************************************************************************************************
