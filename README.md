...
|6|[Mutations_ETL.R](https://github.com/rasiimwe/Code_Examples/blob/main/Mutations_ETL.R)|Most data that I worked with was extracted from variant callers (tools/software used to detect variants/mutations from sequencing data) that store their data in vcf or text files. The structure of these large files is complex (largely unstructured), data values are not atomic, the data is not linked to other data components required for integrated data analyis, which hinders effective and efficient data mining processes. This file contains an example of code used to navigate through server files for specific files of interest, reads the data in each called file, structures or transforms the data and finally bulk-loads it into a back end database. Each file contains thousands of mutations per line, culminating into millions of records in the database. I spent a considerable amount of time optimizing the database to reduce the runtime|
